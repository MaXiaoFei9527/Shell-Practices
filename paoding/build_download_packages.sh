...
# The sample example
# -----------------------------------------------------------------------
# /data/ci/fitout/common/build_download_packages.sh --src /data/docker_down/trident_csc_0.0.4.tar
# /data/ci/fitout/common/build_download_packages.sh --src /data/docker_down/trident_csc_0.0.4.tar --image trident_csc:0.0.4
...
# -----------------------------------------------------------------------
# build_download_packages.sh --src xxx --image xxx
# build_download_packages.sh --src=xxx --image=xxx
...
rmdir /data/docker_down/tmp/${archive_dir} > /dev/null 2>&1
mkdir -vp /data/docker_down/tmp/${archive_dir}
trap "rm -rf /data/docker_down/tmp/${archive_dir}" EXIT
cp ${_arg_src} /data/docker_down/tmp/${archive_dir}
...
cat > /data/docker_down/tmp/${archive_dir}/${install_name} << EOF
#!/bin/bash
...
chmod 755 /data/docker_down/tmp/${archive_dir}/${install_name}
makeself --gzip --follow --nooverwrite /data/docker_down/tmp/${archive_dir} ${dir_name}/${makeself_name} "${commit_message}" ./${install_name}


# -----------------------------------------------------------------------
# Generation Download Links And Package Information
# -----------------------------------------------------------------------
# http://down.paodingai.com/scriber_szse_0.0.28.tar?token=adhYvBsFbM68b0aVq_xbdg&t=1553961600
...
expiration_time=$(date -d "+60 day" +%s)
expiration_code=$(echo -n "cheftin${expiration_time}/${makeself_name}" | openssl md5 -binary | openssl base64 | tr +/ -_ | tr -d =)
paoding_url="http://down.paodingai.com/${makeself_name}?token=${expiration_code}&t=${expiration_time}"

printf '\n%s\n' "========================================"
printf '%3s\t%s\t%s\n' "1" "MD5" "${md5_sum}"
printf '%3s\t%s\t%s\n' "2" "EXP" "${expiration_time}"
printf '%3s\t%s\t%s\n' "3" "URL" "${paoding_url}"
printf '%s\n' "========================================"
